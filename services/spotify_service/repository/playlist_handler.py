...
from services.spotify_service.repository.base_handler import SpotifyRequestHandler
...
class PlaylistHandler(SpotifyRequestHandler):
    ...
    def __validate_pagination_params(self, limit: int, offset: int) -> None:
        ...
        if not (0 <= offset <= 100000):
            logger.error(
                "Invalid offset value: %d. Must be between 1 and 100000.", offset
            )
    ...
    async def get_current_user_playlists(
        ...
        try:
            ...
            logger.info(
                "Successfully fetched (%d) user playlists", len(playlist_page.items)
            )
    ...
    async def create_new_playlist(
        self, name: str, is_public: bool, description: str, is_collaborative: bool
    ) -> Playlist:
        ...
        try:
            ...
            logger.info(
                "Successfully created playlist '%s' for user %s.", name, self.__user_id
            )
            return playlist

        except Exception as e:
            logger.error(
                "Failed to create playlist for user %s: %s", self.__user_id, str(e)
            )
            raise e
    ...
    async def update_playlist_items(
        ...
        logger.debug(
            "Updating playlist %s with (%d) items.", playlist_id, len(item_uris)
        )
