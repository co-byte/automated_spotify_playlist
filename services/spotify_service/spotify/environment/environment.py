...
from spotify.environment.environment_types import EnvironmentTypes


logger = get_logger(__name__)

_DEV_ENV_PATH = "spotify\environment\.env"
...
class Environment:
    def __init__(self, environment: EnvironmentTypes) -> None:
        ...
        if self.__env == EnvironmentTypes.DEVELOPMENT:
            ...
            logger.info(
                "Loading environment variables from local .env file '%s'.",
                _DEV_ENV_PATH,
            )
    ...
    def __update_env_file(self, key: str, value: str) -> None:
        ...
        try:
            ...
            lines = []
            with open(
                self.__env_file_path, "r", encoding=_ENV_FILE_ENCODING
            ) as env_file:
                found = False
            ...
            with open(
                self.__env_file_path, "w", encoding=_ENV_FILE_ENCODING
            ) as env_file:
