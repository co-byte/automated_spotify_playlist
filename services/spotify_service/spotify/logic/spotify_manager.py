...
from spotify.logic.spotify_manager_config import SpotifyManagerConfig
from spotify.models.external_track import ExternalTrack
from spotify.models.page.playlist_page import PlaylistsPage
...
class SpotifyManager:
    ...
    async def __playlists_page_contains_managed_playlist(
        self, limit: int, offset: int
    ) -> bool:
        try:
            page: PlaylistsPage = (
                await self.__config.playlist_handler.get_current_user_playlists(
            ...
            if (
                not self.__config.environment.spotify_playlist_id
                or self.__config.environment.spotify_playlist_id == ""
            ):
                logger.debug(
                    "No managed playlist ID provided. Assuming playlist does not exist."
                )
                return False

            # Check if managed_playlist is present in the current page
            for playlist in playlists:
                if playlist.id == self.__config.environment.spotify_playlist_id:
                    logger.debug(
                        "Existing automated playlist found (ID: %s).", playlist.id
                    )
                    return True
            ...
            return self.__playlists_page_contains_managed_playlist(
    ...
    async def __automated_playlist_exists(self) -> bool:
        try:
            ...
            playlist_exists = await self.__playlists_page_contains_managed_playlist(
                initial_limit, initial_offset
            )
    ...
    async def __update_playlist_with_tracks(self, tracks: List[ExternalTrack]) -> None:
        ...
        await self.__config.playlist_handler.update_playlist_items(
            playlist_id=self.__config.environment.spotify_playlist_id,
    ...
    async def update_managed_playlist(self, tracks: List[ExternalTrack]) -> None:
        try:
            if not await self.__automated_playlist_exists():
                ...
                self.__config.environment.spotify_playlist_id = new_playlist.id
            else:
